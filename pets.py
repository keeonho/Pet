...
def kb_reply() -> ReplyKeyboardMarkup:
    ...
    return ReplyKeyboardMarkup([
        ...
        [KeyboardButton("⚙️ Settings"), KeyboardButton("💳 Top Up Koin")],
...
def kb_pet(pet: dict, pet_lv: int = 1) -> InlineKeyboardMarkup:
    ...
    # Lv 40: Punya anak (hanya kalau sudah nikah)
    if lv >= LEVEL_CHILD and pet.get("is_married"):
        ...
        # Legacy callback: tombol lama pakai format "solo_DEL{code}"
            ...
            try:
                ...
                if member.status in ("member", "administrator", "creator"):
                    ...
                    await context.bot.send_message(
                        user.id,
                        f"🐾 Halo <b>{safe_html(user.first_name)}</b>! Kamu sudah bisa pakai bot sekarang~",
                        parse_mode=ParseMode.HTML,
...
async def answer_catch(q, user, context, chosen: int):
    ...
    if correct is None:
        await q.edit_message_text(
            "⏰ Sesi game sudah habis, mulai lagi ya!",
            parse_mode=ParseMode.HTML,
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚽ Main Lagi", callback_data="game_catch"), InlineKeyboardButton("🔙", callback_data="game_menu")]])
        )
        return
    if chosen == correct:
...
async def error_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if isinstance(err, (TimedOut, NetworkError)):
        logger.warning(f"Network/timeout error (ignored): {err}")
        return
    if isinstance(err, RetryAfter):
        logger.warning(f"RetryAfter {err.retry_after}s")
        await asyncio.sleep(err.retry_after)
        return
    if isinstance(err, BadRequest) and any(x in str(err) for x in ("Message is not modified", "query is too old", "MESSAGE_ID_INVALID")):
        return
